            ...
            <div class="table_block" id="table_block">
                ...
                <div class="scroll_block">
                    <table class="scroll_table" id="scroll_table">
                        <thead>
                            <?php
                                $grades_count = $result_sql->num_rows;
                                if ($grades_count > 0) {
                        ...
                        <tbody>
                            <?php
                                $sql_s = "SELECT * FROM students WHERE user_id = " . $_SESSION['user_id'];
                                $res_s = $conn->query($sql_s);
                                if ($res_s->num_rows > 0) {
                                    while ($row_s = $res_s->fetch_assoc()) {
                                        echo "<tr>";
                                        if ($row_s["group_num"] != "-") {
                                            $arr = explode(", ", $row_s["grades"]);
                                            for ($col = 0; $col < count($arr); $col++) {
                                                if ($arr[$col] == '-') {
                                                    echo "<td><a href='#' class='grade_button' data_student_id='" . $row_s["id"] . "' data_grade_index='" . $col . "'>+</a></td>";
                                                } else if ($arr[$col] == '*') {
                                                    echo "<td class='task_for_group' title='За это задание нельзя поставить оценку, так как ученик не в группе'>+</td>";
                                                } else if ($arr[$col] >= 1 && $arr[$col] <= 5) {
                                                    echo "<td><a href='#' class='grade_button' data_student_id='" . $row_s["id"] . "' data_grade_index='" . $col . "'>" . $arr[$col] ."</a></td>";
                                                }
                                            }
                                        } else {
                                            $arr = explode(", ", $row_s["grades"]);
